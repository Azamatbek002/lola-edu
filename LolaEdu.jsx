...
import { useSupabase } from './src/useData.js';
...
export default function App() {
  ...
  const users = sb.users.length ? sb.users : USERS0;
  const setUsers = sb.setUsers;
  const branches = sb.branches.length ? sb.branches : BRANCHES0;
  const setBranches = sb.setBranches;
  const groups = sb.groups.length ? sb.groups : GROUPS0;
  const setGroups = sb.setGroups;
  const courses = sb.courses.length ? sb.courses : COURSES0;
  const setCourses = sb.setCourses;
  const attendance = sb.attendance.length ? sb.attendance : ATTENDANCE0;
  const setAttendance = sb.setAttendance;
  const payments = sb.payments.length ? sb.payments : PAYMENTS0;
  const setPayments = sb.setPayments;
  const expenses = sb.expenses.length ? sb.expenses : EXPENSES0;
  const setExpenses = sb.setExpenses;
  const leads = sb.leads.length ? sb.leads : LEADS0;
  const setLeads = sb.setLeads;
  const grades = sb.grades.length ? sb.grades : GRADES0;
  const setGrades = sb.setGrades;
  const salaries = sb.salaries.length ? sb.salaries : SALARIES0;
  const setSalaries = sb.setSalaries;
  const notifications = sb.notifications.length ? sb.notifications : NOTIFICATIONS0;
  const setNotifications = sb.setNotifications;
  const smsLog = sb.smsLog.length ? sb.smsLog : SMSLOG0;
  const setSmsLog = sb.setSmsLog;
