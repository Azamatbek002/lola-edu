...
const SB_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9hZ252d21hcWZ2dXl0c2JxanFtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2MzE2ODIsImV4cCI6MjA5NjIwNzY4Mn0.pQ_sUwu5t-58w_I2BAykZpu9Jud0542gzpaLOw2LX60';
...
const STYLE = `
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .btn-green{background:var(--green);color:#fff;border:none;padding:8px 16px;border-radius:8px;font-size:0.83rem;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;}
  ...
  .notif-btn{position:relative;background:none;border:none;cursor:pointer;font-size:1.25rem;padding:6px;border-radius:8px;}
  ...
  .grade-circle{width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:0.9rem;}
...
const BRANCHES0=[
  {id:"b1",name:"Lola Edu Chilonzor",address:"Toshkent, Chilonzor",phone:"+998712223344",directorId:"u2"},
  {id:"b2",name:"Lola Edu Sergeli",address:"Toshkent, Sergeli",phone:"+998712223355",directorId:"u13"},
];
const COURSES0=[
  {id:"c1",name:"Ingliz tili (IELTS)",duration:"4 oy",price:500000,description:"IELTS 6.0+"},
...
const EXPENSES0=[
  {id:"e1",category:"Ijara",amount:3000000,date:"2024-05-01",note:"Ofis ijarasi",branchId:"b1"},
  {id:"e2",category:"Kommunal",amount:400000,date:"2024-05-05",note:"Elektr, suv",branchId:"b1"},
];
const LEADS0=[
  {id:"l1",name:"Aziza Xasanova",phone:"+998901230001",source:"Instagram",course:"Ingliz tili",stage:"new",note:"",date:"2024-05-01",assignedTo:"u7",branchId:"b1"},
  {id:"l2",name:"Temur Yusupov",phone:"+998901230002",source:"Telegram",course:"Matematika",stage:"called",note:"",date:"2024-05-02",assignedTo:"u7",branchId:"b1"},
  {id:"l3",name:"Gulnora Mirzaeva",phone:"+998901230003",source:"Referral",course:"Rus tili",stage:"trial",note:"",date:"2024-05-03",assignedTo:"u3",branchId:"b1"},
];
const GRADES0=[
  {id:"gr1",studentId:"u8",groupId:"g1",type:"homework",title:"Unit 4",score:85,maxScore:100,date:"2024-04-20",teacherNote:"Yaxshi"},
  {id:"gr2",studentId:"u9",groupId:"g2",type:"quiz",title:"Grammar Quiz",score:72,maxScore:100,date:"2024-04-22",teacherNote:""},
];
const SALARIES0=[
...
const NOTIFICATIONS0=[
  {id:"n1",text:"Zulfiya bugun darsga kelmadi",time:"09:15",type:"warning",read:false,forRoles:["admin","reception","superadmin","manager"]},
  {id:"n2",text:"Yangi lead: Aziza Xasanova",time:"10:00",type:"info",read:false,forRoles:["admin","manager","superadmin"]},
];
const SMSLOG0=[];
...
const NAV={
  ...
  branch_dir:[
    ...
    {section:"Boshqaruv"},{key:"leads",ico:"🎯",label:"Leadlar"},{key:"attendance",ico:"✅",label:"Davomat"},
  ...
  reception:[
    {section:"Asosiy"},{key:"dashboard",ico:"📊",label:"Dashboard"},
    {section:"O'quv"},{key:"students",ico:"👩‍🎓",label:"Talabalar"},{key:"attendance",ico:"✅",label:"Davomat"},
...
const PAGE_TITLES={
  ...
  expenses:"Xarajatlar",salary:"Maosh",reports:"Hisobotlar",users:"Foydalanuvchilar",
  ...
  my_grades:"Baholarim",my_payments:"To'lovlarim",
  child_attendance:"Davomat",child_grades:"Baholar",child_payments:"To'lovlar",
};
    ...
    <div style={{position:"relative"}} ref={ref}>
      ...
      {open&&(
        <div className="notif-panel">
          ...
          {mine.length===0?<div className="empty">Yo'q</div>:mine.slice(0,8).map(n=>(
...
function StatCard({ico,val,lbl,ac}){
  return(
    <div className="stat-card" style={{"--accent":ac}}>
      <div className="ico2">{ico}</div>
      <div className="val" style={{fontSize:typeof val==="string"&&val.includes("so'm")?"0.95rem":"1.7rem"}}>{val}</div>
      <div className="lbl">{lbl}</div>
    </div>
  );
}

function AdminDash({users,groups,payments,expenses,leads,branches}){
  const students=users.filter(u=>u.role==="student");
  const income=payments.filter(p=>p.status==="paid").reduce((s,p)=>s+p.amount,0);
  const exp=expenses.reduce((s,e)=>s+e.amount,0);
  const newLeads=leads.filter(l=>l.stage==="new").length;
  const months=["Jan","Feb","Mar","Apr","May","Jun"];
  const mi=months.map(()=>Math.round(Math.random()*4000000+1500000));
  const mx=Math.max(...mi);
  return(
    <div className="page-fade">
      <div className="stat-grid">
        <StatCard ico="👩‍🎓" val={students.length} lbl="Talabalar" ac="#3b82f6"/>
        <StatCard ico="📚" val={groups.length} lbl="Guruhlar" ac="#f59e0b"/>
        <StatCard ico="💰" val={fmt(income)} lbl="Daromad" ac="#10b981"/>
        <StatCard ico="💸" val={fmt(exp)} lbl="Xarajatlar" ac="#ef4444"/>
        <StatCard ico="🎯" val={newLeads} lbl="Yangi leadlar" ac="#C8102E"/>
        <StatCard ico="🏢" val={branches.length} lbl="Filiallar" ac="#8b5cf6"/>
      </div>
      <div className="card">
          ...
          <div className="bar-chart">
            {mi.map((v,i)=>(
              <div key={i} className="bar-col">
                <div style={{fontSize:"0.67rem",color:"var(--muted)",fontWeight:700}}>{(v/1000000).toFixed(1)}M</div>
                <div className="bar" style={{height:`${(v/mx)*90}px`}}/>
                <div style={{fontSize:"0.67rem",color:"var(--muted)"}}>{months[i]}</div>
              </div>
            ))}
          </div>
        </div>
...
function TeacherDash({cu,groups,attendance}){
  ...
  return(
    <div className="page-fade">
      <div className="stat-grid">
        <StatCard ico="📚" val={mg.length} lbl="Guruhlarim" ac="#3b82f6"/>
        <StatCard ico="👩‍🎓" val={msi.length} lbl="Talabalar" ac="#10b981"/>
        <StatCard ico="✅" val={`${ta.filter(a=>a.status==="present").length}/${ta.length}`} lbl="Bugungi davomat" ac="#C8102E"/>
      </div>
      <div className="card">
        <div className="card-hd"><h3>📚 Guruhlarim</h3></div>
        <table><thead><tr><th>Guruh</th><th>Jadval</th><th>Xona</th></tr></thead>
          <tbody>{mg.map(g=>(
            <tr key={g.id}>
              <td><strong>{g.name}</strong></td>
              <td style={{fontSize:"0.82rem",color:"var(--muted)"}}>{g.schedule}</td>
              <td>{g.room}</td>
            </tr>
          ))}</tbody>
        </table>
...
function ParentDash({cu,users,groups,attendance,payments,grades}){
  ...
  return(
    <div className="page-fade">
      ...
      <div className="stat-grid">
        <StatCard ico="✅" val={pct+"%"} lbl="Davomat" ac={pct>=70?"#10b981":"#C8102E"}/>
        <StatCard ico="📝" val={avg+"%"} lbl="O'rtacha baho" ac="#3b82f6"/>
        <StatCard ico={unp.length?"⚠️":"✅"} val={unp.length?fmt(unp.reduce((s,p)=>s+p.amount,0)):"To'liq"} lbl="To'lov" ac={unp.length?"#ef4444":"#10b981"}/>
      </div>
    </div>
  );
}
...
function StudentDash({cu,groups,payments,attendance,courses}){
  ...
  return(
    <div className="page-fade">
      <div className="stat-grid">
        <StatCard ico="📚" val={group?.name||"—"} lbl="Guruhim" ac="#3b82f6"/>
        <StatCard ico="✅" val={pct+"%"} lbl="Davomat" ac={pct>=70?"#10b981":"#C8102E"}/>
        <StatCard ico={unp.length?"⚠️":"✅"} val={unp.length?fmt(unp.reduce((s,p)=>s+p.amount,0)):"To'liq"} lbl="To'lov" ac={unp.length?"#ef4444":"#10b981"}/>
      </div>
      {group&&<div className="card" style={{padding:"18px"}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
        {[["Guruh",group.name],["Kurs",course?.name||"—"],["Jadval",group.schedule],["Xona",group.room],["Narx",fmt(course?.price||0)],["Tugash",group.endDate]].map(([k,v])=>(
...
function StudentsPage({users,setUsers,groups,payments,cu}){
      ...
      <div className="card">
        <table>
          ...
          <tbody>
            ...
            {filtered.length===0&&<tr><td colSpan={5}><div className="empty">Topilmadi</div></td></tr>}
          </tbody>
        </table>
      </div>
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            ...
            {[["name","Ism"],["email","Email"],["phone","Telefon"],["password","Parol"]].map(([f,l])=>(
  ...
  return(
    <div className="page-fade">
          ...
          <tbody>{vis.map(g=>{
            ...
            return(
              <tr key={g.id}>
                ...
                <td><span className="badge blue">{g.students?.length||0} ta</span></td>
      ...
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            <h3>{modal==="new"?"Yangi guruh":"Tahrirlash"}</h3>
  ...
  return(
    <div className="page-fade">
        ...
        {courses.map(c=>(
          <div key={c.id} className="card" style={{padding:"18px"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
              ...
              <div><div style={{fontSize:"0.73rem",color:"var(--muted)"}}>Narxi</div><div style={{fontWeight:800,fontSize:"0.95rem",color:"var(--red)"}}>{fmt(c.price)}</div></div>
            </div>
      ...
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            <h3>{modal==="new"?"Yangi kurs":"Tahrirlash"}</h3>
            {[["name","Nom"],["duration","Davomiyligi"],["price","Narxi"],["description","Tavsif"]].map(([f,l])=>(
              <div key={f} className="fg"><label>{l}</label><input value={form[f]||""} onChange={e=>setForm(p=>({...p,[f]:e.target.value}))}/></div>
            ))}
            <div className="ma"><button className="btn-ghost" onClick={()=>setModal(null)}>Bekor</button><button className="btn-red" onClick={save}>Saqlash</button></div>
          </div>
...
function LeadsPage({leads,setLeads,users,courses,setUsers,cu}){
  const [modal,setModal]=useState(null);
  ...
  const vis=["superadmin","admin"].includes(cu.role)?leads:leads.filter(l=>l.branchId===cu.branchId);
  ...
  const move=(id,stage)=>setLeads(p=>p.map(l=>l.id===id?{...l,stage}:l));
  const convert=(lead)=>{
    setUsers(p=>[...p,{id:"u"+uid(),name:lead.name,email:lead.phone+"@lola.uz",password:"1234",role:"student",phone:lead.phone,branchId:lead.branchId}]);
    ...
    <div className="page-fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",flexWrap:"wrap",gap:"10px"}}>
        <div className="flex gap2" style={{flexWrap:"wrap"}}>
          {STAGES.map(s=><div key={s.key} style={{background:"var(--card)",border:`2px solid ${s.color}`,borderRadius:"9px",padding:"5px 11px",fontSize:"0.78rem",fontWeight:700,color:s.color}}>{s.label}: {vis.filter(l=>l.stage===s.key).length}</div>)}
        </div>
      ...
      <div className="pipeline">
        {STAGES.map(s=>(
          <div key={s.key} className="pipeline-col">
            <div className="pipeline-col-hd"><span style={{color:s.color}}>{s.label}</span><span className="badge" style={{background:s.color+"20",color:s.color}}>{vis.filter(l=>l.stage===s.key).length}</span></div>
            {vis.filter(l=>l.stage===s.key).map(l=>(
            ...
            {vis.filter(l=>l.stage===s.key).length===0&&<div style={{textAlign:"center",padding:"20px",color:"var(--muted)",fontSize:"0.8rem"}}>Bo'sh</div>}
          </div>
        ))}
      </div>
      {detail&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setDetail(null)}>
          <div className="modal">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"16px"}}>
              <h3 style={{marginBottom:0}}>{detail.name}</h3>
              <button className="btn-ghost btn-sm" onClick={()=>setDetail(null)}>✕</button>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"16px"}}>
              {[["📱 Telefon",detail.phone],["📌 Manba",detail.source],["📚 Kurs",detail.course],["📅 Sana",detail.date]].map(([k,v])=>(
            ...
            <div style={{marginBottom:"14px"}}>
              <div style={{fontSize:"0.79rem",color:"var(--muted)",fontWeight:700,marginBottom:"7px"}}>Bosqich:</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:"6px"}}>
                {STAGES.map(s=>(
                  <button key={s.key} onClick={()=>{move(detail.id,s.key);setDetail(p=>({...p,stage:s.key}));}}
                    style={{padding:"5px 11px",borderRadius:"8px",border:`2px solid ${s.color}`,background:detail.stage===s.key?s.color:"transparent",color:detail.stage===s.key?"#fff":s.color,fontSize:"0.78rem",fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
              <button className="btn-ghost btn-sm" onClick={()=>{setDetail(null);open(detail);}}>✏️ Tahrirlash</button>
              <button className="btn-green btn-sm" onClick={()=>convert(detail)}>🎓 Talabaga o'tkazish</button>
              <button className="btn-red btn-sm" onClick={()=>{del(detail.id);setDetail(null);}}>🗑️ O'chirish</button>
            </div>
          </div>
        </div>
      )}
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            <h3>{modal==="new"?"Yangi lead":"Tahrirlash"}</h3>
            <div className="fg2">
              <div className="fg"><label>Ism</label><input value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div>
              <div className="fg"><label>Telefon</label><input value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value}))}/></div>
            </div>
            <div className="fg2">
              <div className="fg"><label>Manba</label><select value={form.source} onChange={e=>setForm(p=>({...p,source:e.target.value}))}>{["Instagram","Telegram","Referral","Website","Boshqa"].map(s=><option key={s}>{s}</option>)}</select></div>
            ...
            <div className="fg"><label>Izoh</label><textarea value={form.note} onChange={e=>setForm(p=>({...p,note:e.target.value}))} rows={2} style={{width:"100%",padding:"9px",border:"1px solid var(--border)",borderRadius:"8px",fontSize:"0.85rem",fontFamily:"'DM Sans',sans-serif",background:"var(--bg)"}}/></div>
            <div className="ma"><button className="btn-ghost" onClick={()=>setModal(null)}>Bekor</button><button className="btn-red" onClick={save}>Saqlash</button></div>
          </div>
        </div>
      )}
...
function AttendancePage({attendance,groups,users,cu}){
  ...
  const sg=selGroup||vis[0]?.id||"";
  const group=vis.find(g=>g.id===sg);
  const records=attendance.filter(a=>a.groupId===sg&&a.date===selDate);
  const students=(group?.students||[]).map(id=>users.find(u=>u.id===id)).filter(Boolean);
  return(
    <div className="page-fade">
      <div style={{display:"flex",gap:"10px",marginBottom:"16px",flexWrap:"wrap"}}>
        <select value={sg} onChange={e=>setSelGroup(e.target.value)} style={{padding:"8px 13px",border:"1px solid var(--border)",borderRadius:"8px",fontSize:"0.84rem",fontFamily:"'DM Sans',sans-serif",background:"var(--card)"}}>
      ...
      <div className="card">
        <div className="card-hd"><h3>✅ {group?.name} | {selDate}</h3>
        ...
        <table><thead><tr><th>Talaba</th><th>Holat</th></tr></thead>
  ...
  const save=()=>{
    ...
    if(bad.length){
      setNotifications(p=>[{id:"n"+uid(),forRoles:["admin","reception","superadmin","manager"],
        text:`${cu.name}: ${bad.map(s=>s.name).join(", ")} ${bad.length>1?"kelmadi/kech keldi":"kelmadi"}`,
              ...
              <td><div className="flex gap2">
                {[["present","✅ Keldi","#10b981"],["absent","❌ Kelmadi","#ef4444"],["late","🕐 Kech","#f59e0b"]].map(([v,l,c])=>(
                  <button key={v} onClick={()=>setMarks(m=>({...m,[s.id]:v}))}
                    style={{padding:"5px 11px",borderRadius:"7px",border:`2px solid ${(marks[s.id]||"present")===v?c:"var(--border)"}`,background:(marks[s.id]||"present")===v?c+"20":"transparent",color:(marks[s.id]||"present")===v?c:"var(--muted)",fontSize:"0.77rem",fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
                    {l}
                  </button>
                ))}
              </div></td>
...
function GradesPage({grades,setGrades,users,groups,cu}){
  ...
  const visGrades=cu.role==="teacher"?grades.filter(g=>visGroups.map(gr=>gr.id).includes(g.groupId)):grades;
  ...
  return(
    <div className="page-fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"}}>
        <div/>{canEdit&&<button className="btn-red" onClick={()=>open(null)}>+ Baho qo'shish</button>}
      </div>
      <div className="tabs">{GRADE_TYPES.map(t=><button key={t.k} className={`tab ${tab===t.k?"active":""}`} onClick={()=>setTab(t.k)}>{t.l}</button>)}</div>
      <div className="card">
        <table><thead><tr><th>Talaba</th><th>Guruh</th><th>Sarlavha</th><th>Ball</th><th>Foiz</th><th>Sana</th>{canEdit&&<th>Amal</th>}</tr></thead>
          <tbody>{filtered.map(g=>{
            ...
            return(
              <tr key={g.id}>
                ...
                {canEdit&&<td><div className="flex gap2"><button className="btn-ghost btn-sm" onClick={()=>open(g)}>✏️</button><button className="btn-red btn-sm" onClick={()=>setGrades(p=>p.filter(x=>x.id!==g.id))}>🗑️</button></div></td>}
              </tr>
            );
          })}
          {filtered.length===0&&<tr><td colSpan={7}><div className="empty">Baholar yo'q</div></td></tr>}
          </tbody>
        </table>
      </div>
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            <h3>{modal==="new"?"Yangi baho":"Tahrirlash"}</h3>
            <div className="fg2">
              <div className="fg"><label>Guruh</label><select value={form.groupId} onChange={e=>setForm(p=>({...p,groupId:e.target.value}))}><option value="">Tanlang</option>{visGroups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select></div>
              <div className="fg"><label>Talaba</label><select value={form.studentId} onChange={e=>setForm(p=>({...p,studentId:e.target.value}))}><option value="">Tanlang</option>{(groups.find(g=>g.id===form.groupId)?.students||[]).map(sid=>{const u=users.find(u=>u.id===sid);return u?<option key={sid} value={sid}>{u.name}</option>:null;})}</select></div>
            </div>
            ...
            <div className="fg2">
              ...
              <div className="fg"><label>Maksimal</label><input type="number" value={form.maxScore} onChange={e=>setForm(p=>({...p,maxScore:e.target.value}))}/></div>
            </div>
...
function PaymentsPage({payments,setPayments,users,groups,cu}){
  ...
  return(
    <div className="page-fade">
      <div className="tabs">{[["all","Barchasi"],["unpaid","Qarzdorlar"],["paid","To'langanlar"]].map(([v,l])=>(
        <button key={v} className={`tab ${tab===v?"active":""}`} onClick={()=>setTab(v)}>{l}</button>
      ))}</div>
      <div className="card">
        <table><thead><tr><th>Talaba</th><th>Guruh</th><th>Oy</th><th>Summa</th><th>Usul</th><th>Holat</th><th>Amal</th></tr></thead>
          <tbody>{filtered.map(p=>{
            ...
            return(
              <tr key={p.id}>
                ...
                ):<span className="badge blue">{p.method}</span>}</td>
...
function ExpensesPage({expenses,setExpenses,cu}){
  ...
  const [form,setForm]=useState({category:"Ijara",amount:"",date:today,note:"",branchId:cu.branchId||"b1"});
  const vis=["superadmin","admin"].includes(cu.role)?expenses:expenses.filter(e=>e.branchId===cu.branchId);
  const save=()=>{setExpenses(p=>[...p,{...form,id:"e"+uid(),amount:Number(form.amount)}]);setModal(false);};
  return(
    <div className="page-fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
        <div style={{background:"var(--card)",padding:"11px 18px",borderRadius:"11px",border:"1px solid var(--border)"}}>
          ...
          <span style={{fontWeight:700,color:"var(--red)"}}>{fmt(vis.reduce((s,e)=>s+e.amount,0))}</span>
        </div>
        <button className="btn-red" onClick={()=>setModal(true)}>+ Xarajat qo'shish</button>
      </div>
      <div className="card">
        <table><thead><tr><th>Kategoriya</th><th>Tavsif</th><th>Summa</th><th>Sana</th><th>Amal</th></tr></thead>
          <tbody>{vis.map(e=>(
            <tr key={e.id}>
              <td><span className="badge blue">{e.category}</span></td>
              <td style={{fontSize:"0.85rem"}}>{e.note}</td>
              <td style={{fontWeight:700,color:"var(--red)"}}>{fmt(e.amount)}</td>
              <td style={{fontSize:"0.81rem",color:"var(--muted)"}}>{e.date}</td>
              <td><button className="btn-red btn-sm" onClick={()=>setExpenses(p=>p.filter(x=>x.id!==e.id))}>🗑️</button></td>
            </tr>
          ))}
          {vis.length===0&&<tr><td colSpan={5}><div className="empty">Yo'q</div></td></tr>}
          </tbody>
        </table>
      </div>
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div className="modal">
            <h3>Yangi xarajat</h3>
            <div className="fg"><label>Kategoriya</label><select value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value}))}>{["Ijara","Kommunal","Jihozlar","Maosh","Marketing","Boshqa"].map(c=><option key={c}>{c}</option>)}</select></div>
            <div className="fg"><label>Summa</label><input type="number" value={form.amount} onChange={e=>setForm(p=>({...p,amount:e.target.value}))}/></div>
            <div className="fg"><label>Sana</label><input type="date" value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))}/></div>
            <div className="fg"><label>Tavsif</label><input value={form.note} onChange={e=>setForm(p=>({...p,note:e.target.value}))}/></div>
...
function SalaryPage({salaries,setSalaries,users,groups,payments,cu}){
  ...
  const save=()=>{
    if(modal==="new")setSalaries(p=>[...p,{...form,id:"sal"+uid(),baseAmount:Number(form.baseAmount),bonusAmount:Number(form.bonusAmount),deduction:Number(form.deduction)}]);
    else setSalaries(p=>p.map(s=>s.id===modal.id?{...s,...form}:s));
    setModal(null);
  };
  return(
    <div className="page-fade">
      ...
      <div className="card">
        <table><thead><tr><th>O'qituvchi</th><th>Oy</th><th>Asosiy</th><th>Bonus</th><th>Jarima</th><th>Jami</th><th>Holat</th><th>Amal</th></tr></thead>
          <tbody>{vis.map(s=>{
            ...
            return(
              <tr key={s.id}>
                ...
                <td><div className="flex gap2">
                  ...
                  {s.status==="unpaid"&&<button className="btn-green btn-sm" onClick={()=>setSalaries(p=>p.map(x=>x.id===s.id?{...x,status:"paid"}:x))}>💸 To'la</button>}
                </div></td>
              </tr>
            );
          })}
          {vis.length===0&&<tr><td colSpan={8}><div className="empty">Yo'q</div></td></tr>}
          </tbody>
        </table>
      </div>
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            ...
            <div className="fg"><label>Asosiy maosh</label><input type="number" value={form.baseAmount} onChange={e=>setForm(p=>({...p,baseAmount:e.target.value}))}/></div>
            <div className="fg2">
...
function ReportsPage({payments,expenses,groups,leads}){
          ...
          <div style={{padding:"14px"}}>
            {Object.entries(leadBySrc).map(([src,cnt])=>(
              <div key={src} style={{marginBottom:"10px"}}>
                ...
                <div className="progress-bar"><div className="progress-fill" style={{width:`${(cnt/Math.max(leads.length,1))*100}%`,background:"var(--blue)"}}/></div>
              </div>
            ))}
...
function TelegramPage({tgConfig,setTgConfig,users,payments,attendance,smsLog,setSmsLog,setNotifications}){
  ...
  const send=(to,text,type)=>{
    if(tgConfig.connected){alert(`✈️ Telegram: ${to}\n${text}`);}
    else{alert(`📱 SMS: ${to}\n${text}`);}
    setSmsLog(p=>[{id:"s"+uid(),to,text,date:today,channel:tgConfig.connected?"telegram":"sms",type,status:"sent"},...p]);
  };
  const TEMPLATES=[
    {label:"Davomat",text:"Assalomu alaykum! Bugun darsga kelmadingiz."},
    {label:"To'lov",text:"Assalomu alaykum! To'lov muddatingiz o'tdi."},
    {label:"Dars",text:"Assalomu alaykum! Ertaga dars bor. Vaqt: 09:00"},
  ];
  return(
    <div className="page-fade">
      <div className="tabs">{[["config","Bot sozlash"],["send","Xabar yuborish"],["log","Tarix"]].map(([v,l])=>(
        <button key={v} className={`tab ${tab===v?"active":""}`} onClick={()=>setTab(v)}>{l}</button>
      ))}</div>
      {tab==="config"&&(
        <div style={{maxWidth:"520px"}}>
          <div className="tg-setup">
            ...
            <p>1. @BotFather ga /newbot yuboring<br/>2. Token oling<br/>3. Quyida kiriting</p>
            <input style={{width:"100%",padding:"9px",borderRadius:"8px",border:"none",fontSize:"0.85rem",fontFamily:"'DM Sans',sans-serif",marginTop:"10px"}} value={token} onChange={e=>setToken(e.target.value)} placeholder="Bot token..."/>
          </div>
          <div className="fg"><label>Chat ID</label><input value={chatId} onChange={e=>setChatId(e.target.value)} placeholder="-100123456789"/></div>
          <div style={{display:"flex",gap:"10px",alignItems:"center",marginBottom:"16px"}}>
            <span className={`badge ${tgConfig.connected?"green":"gray"}`}>{tgConfig.connected?"✅ Ulangan":"❌ Ulanmagan"}</span>
          </div>
          <button className="btn-red" onClick={()=>setTgConfig({token,chatId,connected:token.length>10})}>💾 Saqlash</button>
        </div>
      )}
      {tab==="send"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}>
          <div className="card" style={{padding:"18px"}}>
            ...
            <div className="fg"><label>Xabar</label><textarea value={msg} onChange={e=>setMsg(e.target.value)} rows={3} style={{width:"100%",padding:"9px",border:"1px solid var(--border)",borderRadius:"8px",fontSize:"0.85rem",fontFamily:"'DM Sans',sans-serif",background:"var(--bg)"}}/></div>
            ...
            <button className="btn-red" style={{width:"100%"}} onClick={()=>{const s=users.find(u=>u.id===selStudent);if(s&&msg)send(s.phone,msg,"individual");}}>📤 Yuborish</button>
          </div>
          <div className="card" style={{padding:"18px"}}>
            <h3 style={{fontSize:"0.95rem",fontFamily:"'DM Sans',sans-serif",fontWeight:700,marginBottom:"14px"}}>📢 Avtomatik</h3>
            {[{ico:"❌",title:`Kelmaganlar (${absent.length})`,list:absent,text:"Bugun darsga kelmadingiz.",type:"absent"},{ico:"⚠️",title:`Qarzdorlar (${debtors.length})`,list:debtors,text:"To'lov muddatingiz o'tdi.",type:"debt"}].map((item,i)=>(
              <div key={i} style={{padding:"13px",background:"var(--bg)",borderRadius:"10px",border:"1px solid var(--border)",marginBottom:"10px"}}>
                <div style={{fontWeight:700,fontSize:"0.84rem",marginBottom:"8px"}}>{item.ico} {item.title}</div>
                <button className="btn-red btn-sm" onClick={()=>item.list.forEach(u=>send(u.phone,item.text,item.type))}>Barchasiga yuborish</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab==="log"&&(
        <div className="card">
          <table><thead><tr><th>Kimga</th><th>Xabar</th><th>Kanal</th><th>Sana</th></tr></thead>
            <tbody>{smsLog.map(s=>(
              <tr key={s.id}>
                <td style={{fontSize:"0.83rem"}}>{s.to}</td>
            ...
            {smsLog.length===0&&<tr><td colSpan={4}><div className="empty">Tarix yo'q</div></td></tr>}
            </tbody>
          </table>
        </div>
      )}
  ...
  return(
    <div className="page-fade">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:"14px"}}>
        {branches.map(b=>{
          const bs=users.filter(u=>u.role==="student"&&u.branchId===b.id).length;
          const bt=users.filter(u=>u.role==="teacher"&&u.branchId===b.id).length;
          const bg=groups.filter(g=>g.branchId===b.id).length;
          const bi=payments.filter(p=>p.status==="paid"&&users.find(u=>u.id===p.studentId)?.branchId===b.id).reduce((s,p)=>s+p.amount,0);
          ...
          return(
            <div key={b.id} className="card" style={{padding:"20px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginTop:"12px"}}>
                {[["👩‍🎓",bs,"Talaba"],["👨‍🏫",bt,"O'qituvchi"],["📚",bg,"Guruh"],["💰",fmt(bi),"Daromad"]].map(([ico,v,l])=>(
                  <div key={l} style={{background:"var(--bg)",padding:"9px",borderRadius:"9px",textAlign:"center"}}>
                    <div>{ico}</div><div style={{fontWeight:700,fontSize:"0.85rem"}}>{v}</div>
      ...
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            <h3>{modal==="new"?"Yangi filial":"Tahrirlash"}</h3>
            {[["name","Nom"],["address","Manzil"],["phone","Telefon"]].map(([f,l])=>(
              <div key={f} className="fg"><label>{l}</label><input value={form[f]||""} onChange={e=>setForm(p=>({...p,[f]:e.target.value}))}/></div>
            ))}
...
function UsersPage({users,setUsers,branches}){
  ...
  return(
    <div className="page-fade">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",flexWrap:"wrap",gap:"10px"}}>
        <input className="search-input" placeholder="🔍 Qidirish..." value={search} onChange={e=>setSearch(e.target.value)}/>
        <button className="btn-red" onClick={()=>open(null)}>+ Qo'shish</button>
      </div>
      <div className="card">
        <table><thead><tr><th>Foydalanuvchi</th><th>Email</th><th>Rol</th><th>Filial</th><th>Amal</th></tr></thead>
          <tbody>{filtered.map(u=>{
            ...
            return(
              <tr key={u.id}>
                ...
                <td><div className="flex gap2">
                  <button className="btn-ghost btn-sm" onClick={()=>open(u)}>✏️</button>
                  <button className="btn-red btn-sm" onClick={()=>{if(confirm("O'chirishni tasdiqlaysizmi?"))setUsers(p=>p.filter(x=>x.id!==u.id));}}>🗑️</button>
                </div></td>
              </tr>
            );
          })}</tbody>
      ...
      {modal&&(
        <div className="modal-ov" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            ...
            <div className="fg2">
              <div className="fg"><label>Rol</label><select value={form.role} onChange={e=>setForm(p=>({...p,role:e.target.value}))}>{Object.keys(ROLE_LABELS).map(r=><option key={r} value={r}>{ROLE_LABELS[r]}</option>)}</select></div>
        ...
        <table><thead><tr><th>Talaba</th><th>Telefon</th><th>Davomat</th></tr></thead>
          <tbody>{students.map(s=>{const p=pct(s.id);return(
            <tr key={s.id}>
              <td><div className="flex items-center gap2"><div className="avatar">{ini(s.name)}</div>{s.name}</div></td>
              <td style={{color:"var(--muted)",fontSize:"0.83rem"}}>{s.phone}</td>
              <td style={{width:"180px"}}><div style={{display:"flex",alignItems:"center",gap:"9px"}}><div className="progress-bar" style={{flex:1}}><div className="progress-fill" style={{width:`${p}%`,background:p>=70?"var(--green)":"var(--red)"}}/></div><span style={{fontSize:"0.8rem",fontWeight:700}}>{p}%</span></div></td>
            </tr>);})}</tbody>
        </table>
      </div>}
    </div>
  ...
  return(
    <div className="page-fade">
      <div className="stat-grid" style={{marginBottom:"18px"}}>
        <StatCard ico="✅" val={att.filter(a=>a.status==="present").length} lbl="Kelgan" ac="#10b981"/>
        <StatCard ico="❌" val={att.filter(a=>a.status==="absent").length} lbl="Kelmagan" ac="#ef4444"/>
        <StatCard ico="🕐" val={att.filter(a=>a.status==="late").length} lbl="Kech kelgan" ac="#f59e0b"/>
        <StatCard ico="📊" val={pct+"%"} lbl="Umumiy" ac={pct>=70?"#10b981":"#C8102E"}/>
      </div>
...
function MyPaymentsPage({cu,payments}){
  ...
  return(
    <div className="page-fade">
      <div className="stat-grid" style={{marginBottom:"18px"}}>
        <StatCard ico="💳" val={fmt(total)} lbl="Jami" ac="#3b82f6"/>
        <StatCard ico="✅" val={fmt(paid)} lbl="To'langan" ac="#10b981"/>
        <StatCard ico={total-paid>0?"⚠️":"✅"} val={fmt(total-paid)} lbl="Qarzdorlik" ac={total-paid>0?"#ef4444":"#10b981"}/>
      </div>
      <div className="card"><table><thead><tr><th>Oy</th><th>Summa</th><th>Usul</th><th>Holat</th></tr></thead>
        <tbody>{p.map(x=>(
          <tr key={x.id}><td style={{fontWeight:500}}>{x.month}</td><td style={{fontWeight:700}}>{fmt(x.amount)}</td><td>{x.method}</td><td><span className={`badge ${x.status==="paid"?"green":"red"}`}>{x.status==="paid"?"✅ To'langan":"❌ Qarzdor"}</span></td></tr>
        ))}
...
function MyGradesPage({cu,grades,groups}){
  ...
  return(
    <div className="page-fade">
      <div className="stat-grid" style={{marginBottom:"18px"}}>
        <StatCard ico="📝" val={g.length} lbl="Jami baholar" ac="#3b82f6"/>
        <StatCard ico="⭐" val={avg+"%"} lbl="O'rtacha" ac={avg>=70?"#10b981":"#C8102E"}/>
      </div>
      <div className="card"><table><thead><tr><th>Tur</th><th>Sarlavha</th><th>Ball</th><th>Foiz</th><th>Sana</th></tr></thead>
        <tbody>{g.sort((a,b)=>b.date.localeCompare(a.date)).map(x=>{const pct=Math.round((x.score/x.maxScore)*100);return(
          ...
          {g.length===0&&<tr><td colSpan={5}><div className="empty">Baholar yo'q</div></td></tr>}
        </tbody>
      </table></div>
    </div>
  );
...
function ChildAttendancePage({cu,users,attendance,groups}){
  const child=users.find(u=>u.id===cu.childId);
  return <MyAttendancePage cu={child||cu} attendance={attendance} groups={groups}/>;
}
function ChildGradesPage({cu,users,grades,groups}){
  const child=users.find(u=>u.id===cu.childId);
  return <MyGradesPage cu={child||cu} grades={grades} groups={groups}/>;
}
function ChildPaymentsPage({cu,users,payments}){
  const child=users.find(u=>u.id===cu.childId);
  return <MyPaymentsPage cu={child||cu} payments={payments}/>;
}
  ...
  return(
    <div className="page-fade">
      <div className="card" style={{padding:"22px",maxWidth:"500px"}}>
        ...
        {[["Markaz nomi","Lola Edu"],["Manzil",branches[0]?.address||""],["Telefon",branches[0]?.phone||""],["Email","info@lola.uz"]].map(([k,v])=>(
...
export default function App(){
  ...
  const setGroups = makeSetter('groups', setGroupsState);
  const setCourses = makeSetter('courses', setCoursesState);
  const setAttendance = makeSetter('attendance', setAttendanceState);
  const setPayments = makeSetter('payments', setPaymentsState);
  const setExpenses = makeSetter('expenses', setExpensesState);
  const setLeads = makeSetter('leads', setLeadsState);
  const setGrades = makeSetter('grades', setGradesState);
  const setSalaries = makeSetter('salaries', setSalariesState);
  const setNotifications = makeSetter('notifications', setNotificationsState);
  const setSmsLog = makeSetter('sms_log', setSmsLogState);
  ...
  const renderPage=()=>{
    switch(page){
      case "dashboard":
        if(["superadmin","admin","finance","branch_dir"].includes(cu.role)) return <AdminDash {...props}/>;
        if(["manager","reception"].includes(cu.role)) return <AdminDash {...props}/>;
